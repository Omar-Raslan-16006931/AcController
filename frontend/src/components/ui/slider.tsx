    ...
    >
      ...
      {Array.from({ length: values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="border-primary bg-background block size-5 shrink-0 rounded-full border-2 shadow-sm transition-colors focus-visible:ring-4 focus-visible:ring-ring/50 focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  )
